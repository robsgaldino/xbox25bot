...
| 📱 Push no celular (ntfy) | ✅ funcionando | instale o app **ntfy**, escolha um tópico privado e difícil de adivinhar, e coloque-o em `config.json` → `ntfy.topic` (no CI, use o secret `NTFY_TOPIC`) |
| 💬 WhatsApp | ⚙️ falta só o apikey | adicione **+34 644 71 81 99** aos contatos, mande "I allow callmebot to send me messages" no WhatsApp, cole o telefone e o apikey em `config.json` → `whatsapp_callmebot` e mude `enabled` para `true` |
| 📲 SMS | ⚙️ falta conta Twilio | conta trial grátis em twilio.com; preencha SID, token, número de origem e número de destino em `config.json` |
| 📧 E-mail | ⚙️ falta senha de app | [senha de app do Gmail](https://myaccount.google.com/apppasswords) em `config.json` → `email`, junto com o endereço de destino |
